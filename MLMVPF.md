...
### 🏗️ Current Architecture: Temporal Window Analysis System
**Temporal Segmentation Strategy**
- Fixed Windows: Hook (0-3s) and Closing (last 3s) - universal across all video lengths
- Adaptive Middle Segments: Based on MIDDLE DURATION, not total video duration
    - Videos ≤6s: No middle segments (too short for hook+closing+middle)
    - Middle duration 1-2.9s: No segments (middle exists but not segmented)
    - Middle duration 3-12s: 3 segments
    - Middle duration 13-27s: 4 segments
    - Middle duration >27s: 5 segments
- Maximum: 7 windows total (1 hook + 5 middle + 1 closing)
**Segmentation**: 4 duration buckets (0-15s, 16-30s, 31-60s, 61-120s)

### 📊 Feature Engineering Pipeline
Feature Extraction: 50+ features per temporal window including:
- Creative density metrics (element counts, density variations)
- Behavioral indicators (gestures, eye contact, expressions)
- Audio characteristics (speech coverage, energy patterns, pitch)
- Visual elements (text overlays, objects, scene changes)
- Engagement metadata (likes, views, shares from TikTok API)
