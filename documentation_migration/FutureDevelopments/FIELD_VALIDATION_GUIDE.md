...
### 3.3 Stage 7 Quantitative Analysis Output (Winning Formulas)

**Source**: Stage 7 LLM Analysis
**Documentation**: `LLMAnalysis7.md`, `MLPlanningv2.md` (Section 7)
**File Location**: `{bucket_path}/ml_analysis/llm/winning_formulas.json`
  ...
  "creative_reports": [
    {
      ...
      "structure": {
        "hook": "Steady Gaze Hook (Cluster 1)",
        "middle_1": "Silent Visual Storyteller (Cluster 2)",
        "closing": "Visual-First Silent Closer (Cluster 1)"
      },
...
**Characteristics**:
- ✅ 3 creative formulas per bucket (always)
- ✅ Formula names dynamically generated by LLM
- ✅ Aggregated across all videos in bucket (e.g., 40+ videos)
- ✅ Combines temporal window patterns with ML validation
- ✅ Path-based (cluster journey) or feature-based (universal patterns)

**Used For**:
- Creative formula names in reports (Page 2, Section 4)
- Step-by-step execution templates
- Temporal progression insights

**Key Fields**:
- `creative_reports[].formula_name` → Report display name
- `creative_reports[].type` → "path_based" or "feature_based"
- `creative_reports[].confidence_level` → "very_high", "high", or "moderate"
- `creative_reports[].step_by_step_template[]` → Execution instructions

---

### 3.4 Key Differences Across Data Types

| Aspect | Qualitative (Stage 2.7) | Quantitative (Stage 2) | Winning Formulas (Stage 7) |
|--------|-------------|--------------|----------------------------|
| **Data Type** | Categories, labels | Numbers, metrics | Aggregated patterns + names |
| **Source** | LLM classification | ML model detection | LLM analysis of temporal windows |
| **Example** | "personal_testimony" | 0.67 (energy level) | "The Silent-to-Vocal Engagement Journey" |
| **Granularity** | Per video | Per video | Per bucket (aggregated) |
| **Documentation** | ContentAnalysisCHILDTI.md | SystemArchitecturev2.md | LLMAnalysis7.md |
| **Aggregation** | Frequency count, Top N | Average, sum, range | Pattern detection, formula naming |
